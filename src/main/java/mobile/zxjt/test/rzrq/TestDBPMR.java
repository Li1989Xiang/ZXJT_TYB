 ...
 * 担保品买入测试
 */
public class TestDBPMR extends TestBase {
	private TestJYBase mTest = new TestJYBase();

	@BeforeClass
	public void enterDBPMR() {
		Navigator.navigate("担保品买入", mTest.getPage());
	}
	...
	public void testDBPMR(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		// 输入交易信息
	...
	public void testDBPMR_FL_Code(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
	...
	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	@DataFilter(row = FLFilter.class)
	public void testDBPMR_FL_Price(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		PageJY vPage = mTest.getPage();
		// 输入代码并校验股票名称
