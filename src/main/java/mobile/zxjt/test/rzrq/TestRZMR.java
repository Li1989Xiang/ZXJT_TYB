...
public class TestRZMR extends TestBase {
	...
	public void testRZMR_FL_Code(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
	...
	@Test(dataProvider = DataProviderX.NAME_LAZY, dataProviderClass = DataProviderX.class)
	@DataFilter(row = FLFilter.class)
	public void testRZMR_FL_Price(@Mapper(InfoMapper.class) Info info, CustomRecord record) {
		PageJY vPage = mTest.getPage();
		// 输入代码并校验股票名称
